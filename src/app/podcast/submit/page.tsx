...
export default function PodcastSubmitPage() {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setError('Could not access camera/microphone. Allow camera + mic access and try again.')
    }
  }
  ...
  async function upload(blob: Blob) {
    ...
    try {
      const u = await fetch('/api/podcast/upload-url', { method: 'POST' }).then(r => r.json())
      ...
      const reg = await fetch('/api/podcast/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path: u.path, title, consent_ack: true, duration: elapsed }) }).then(r => r.json())
  ...
  if (access === null) return <div className="p-8 text-gray-500">Checking eligibility…</div>
  ...
  if (done) {
    return (
      <div className="p-8 max-w-lg mx-auto text-center">
        ...
        <p className="text-sm text-gray-600 mt-2">Your video was sent to the school office for review. It will appear on the podcast once approved.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">🎬 Record a podcast video</h1>
      <p className="text-sm text-gray-500 mb-4">Use your camera + microphone to record a short podcast clip. It stays private until the school reviews and approves it.</p>
      ...
      <label className="flex items-start gap-2 mb-4 text-sm text-gray-700">
        ...
        <span>I have my parent/guardian's permission to record and share this video with the school for the podcast.</span>
      </label>
      ...
      <div className="flex gap-3">
        {!recording ? (
          <button onClick={startRecording} disabled={!consent} className="flex-1 py-3 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 disabled:opacity-40">🎥 Record</button>
        ) : (
